layui.use(['layer', 'form', 'element', 'jquery'], function() {
	...
	element.on('nav(leftNav)', function(elem) {
		...
		if(isActive.length > 0) {
			...
			element.tabAdd('tab', {
				title: text,
				content: '<iframe src="' + url + '" name="iframe' + id + '" class="iframe" framborder="0" data-id="' + id + '" scrolling="auto" style="width: 100%; height: 100%;"></iframe>',
				id: id
			});
			element.tabChange('tab', id);
	...
	hideBtn.on('click', function() {
		if(!mainLayout.hasClass('layui-hide')) {
			mainLayout.addClass('layui-hide');
		} else {
			mainLayout.removeClass('layui-hide');
		}
	});
	//遮罩点击隐藏
